...
export const getEmployeePerformance = async (userId) => {
    ...
    const satisfactionScores = shifts.filter((s) => s.customerSatisfaction !== null).map((s) => s.customerSatisfaction);
    const averageCustomerSatisfaction = satisfactionScores.length > 0 ? satisfactionScores.reduce((a, b) => a + b, 0) / satisfactionScores.length : null;
    return {
        totalShifts,
        totalHours,
        totalSales,
...
export const getAllEmployeesPerformance = async () => {
    ...
    const performances = await Promise.all(users.map(async (user) => ({
        user: {
            id: user.id,
            name: user.name,
